...
defmodule ArtifactsMmog.Tools do
  ...
  def setup do
    ToolRegistry.register(:get_character_state, fn %{"name" => name} ->
    ...
    ToolRegistry.register(:move, fn %{"name" => name, "x" => x, "y" => y} ->
    ...
    ToolRegistry.register(:fight, fn %{"name" => name} ->
      {:ok, API.fight(name)}
    end,
    ...
    ToolRegistry.register(:gather, fn %{"name" => name} ->
      {:ok, API.gather(name)}
    end,
    ...
    ToolRegistry.register(:rest, fn %{"name" => name} ->
      {:ok, API.rest(name)}
    end,
